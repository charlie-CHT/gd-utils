...
const FILE_EXCEED_MSG = '您的團隊盤檔數已超限(40萬)，停止複製，請將未複製完成的資料夾移到另一個團隊盤中，再執行一遍複製指令即可接上進度繼續複製'
...
async function gen_count_body ({ fid, type, update, service_account }) {
  async function update_info () {
    const info = await walk_and_save({ fid, update, service_account }) // 這一步已經將fid記錄存入資料庫中了
...
async function count ({ fid, update, sort, type, output, not_teamdrive, service_account }) {
  ...
  if (!update) {
    ...
    if (info) {
      console.log('找到本地緩存資料，緩存時間：', dayjs(info.mtime).format('YYYY-MM-DD HH:mm:ss'))
...
async function real_get_sa_token (el) {
  ...
  // 把gtoken傳遞出去的原因是當某帳號流量用盡時可以依此過濾
...
async function user_choose () {
  const response = await prompts({
    ...
    3、退出
    `,
    validate: value => [1, 2, 3].includes(value) ? true : `必須輸入 1/2/3`
  })
...
// 待解決：如果使用者手動ctrl+c中斷進程，那麼已經發出的請求，就算完成了也不會記錄到本地資料庫中，所以可能產生重複檔（夾）
async function real_copy ({ source, target, name, min_size, update, dncnr, not_teamdrive, service_account, is_server }) {
  ...
  if (record) {
    ...
    if (choice === 'exit') {
      ...
      console.log('待複製的檔數：', files.length)
...
async function copy_files ({ files, mapping, service_account, root, task_id }) {
  ...
  console.log('\n開始複製檔，總數：', files.length)

  const loop = setInterval(() => {
    const now = dayjs().format('HH:mm:ss')
    const message = `${now} | 已複製檔數 ${count} | 排隊中檔數 ${files.length}\n`
    print_progress(message)
  }, 3000)
  ...
  //   const message = `${now} | 已複製檔數 ${count} | 網路請求 進行中${activeCount}/排隊中${pendingCount}`
  //   print_progress(message)
  // }, 1000)
  ...
  while (retry < RETRY_LIMIT) {
    ...
    try {
      ...
      if (message && message.toLowerCase().includes('file limit')) {
        ...
        throw new Error('您的團隊盤檔數已超限，停止複製')
      }
...
async function confirm_dedupe ({ file_number, folder_number }) {
  const answer = await prompts({
    type: 'select',
    name: 'value',
    message: `檢測到同位置下重複檔${file_number}個，重複空目錄${folder_number}個，是否刪除？`,
    choices: [
...
// 將檔或資料夾移入回收站，需要 sa 為 content manager 許可權及以上
...
// 直接刪除檔或資料夾，不會進入回收站，需要 sa 為 manager 許可權
